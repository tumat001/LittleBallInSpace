...
func helper__eyes_look_toward_position(arg_pos : Vector2):
	...
	var diff = old_val - _current_x_perspective_axis_dist
	if diff > 50:
		var duration = 0.20
...
#	var curr_cam_angle = CameraManager.current_cam_rotation
#	var old_val = _current_x_perspective_axis_dist
#	_current_x_perspective_axis_dist = lin_velocity.rotated(curr_cam_angle).x
#	if is_equal_approx(curr_cam_angle, PI/2) or is_equal_approx(curr_cam_angle, -PI/2):
#		_current_x_perspective_axis_dist = -_current_x_perspective_axis_dist
#
#	var calced_offset_x__for_right = _calculate_offset_mag__for_right_eye()
...
func _calculate_offset_mag__for_right_eye():
	if _current_x_perspective_axis_dist > 0:
		return _convert_result_into_ratio_using_num_range(_current_x_perspective_axis_dist, 0, DIST_TO_REACH_MAX_OFFSET) * MAX_OFFSET__SAME_SIDE_EYE
	elif _current_x_perspective_axis_dist < 0:
		return _convert_result_into_ratio_using_num_range(_current_x_perspective_axis_dist, 0, DIST_TO_REACH_MAX_OFFSET) * MAX_OFFSET__DIFF_SIDE_EYE
	else:
		return 0
