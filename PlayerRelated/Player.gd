...
func _on_body_entered__tilemap(body_rid, body, body_shape_index, local_shape_index):
	...
	if !is_same_angle_as_perpend_angle:
		...
		_request_rotate(perpend_angle_with_lowest_distance, coordinate)
...
func _request_rotate(arg_angle, arg_ground_identif):
	...
	_current_player_left_right_move_speed = 0
	_attempt_add_on_ground_count__with_any_indentif(arg_ground_identif)
