...
func _on_after_game_start_init():
	...
	vkp_left.text_for_keypress = orig_text__left % InputMap.get_action_list("ui_left")[1].as_text()
	
	var orig_text__right = vkp_right.text_for_keypress
	vkp_right.text_for_keypress = orig_text__right % InputMap.get_action_list("ui_right")[1].as_text()
